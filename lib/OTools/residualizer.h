...
public:
    ...
    unsigned int build(const bool is_center) {
		...
		if(_is_center == true)
		{
			...
			mean_vector = covarM.colwise().mean();
			//std::cout << mean_vector << std::endl;
			for(unsigned int i = 1; i < _ncols; i++)
			{
				covarM.col(i) -= VectorXd::Constant(_nrows, mean_vector(i));
			}
		}
    ...
    unsigned int residualize(vector < float > & data, const bool yintercept) {
    	...
    	if (PQR.rank() == n_covariates + 1) {
			...
			if (_yintercept == true){
				...
				for (int i = 0; i < e.size(); i ++) data[i] = e(i) + m_coef[0];
			}else{
    			VectorXd e = counts - fitted;
    		...
    		if (_yintercept == true){
				VectorXd e = (counts - fitted);
				for (int i = 0; i < e.size(); i ++) data[i] = (float)e(i) + effects[0];
			}else{
    			VectorXd e = counts - fitted;
			}
    	}
    ...
    unsigned int residualize(float * data, const bool yintercept) {
    	_yintercept = yintercept;
		if (n_covariates == 0) return COV_NCOV;
