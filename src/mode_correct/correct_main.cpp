...
void correct_main(vector < string > & argv) {
	...
	opt_param.add_options()
		...
		("center", "Center covariates prior to residualisation. We advise you to use it if you have merged multiple covariates together.")
		("yintercept", "Add back the mean (yintercept) to residuals. We advise you to use it if you are not planning to rank normal transform your data");
	...
	if (D.options.count("bed")) {
		...
		if (D.options.count("center")) { vrb.bullet("Center covariates prior to residualisation step"); D.center = true; }
		if (D.options.count("yintercept")) { vrb.bullet("Add yintercept (mean) to residuals"); D.yintercept = true; }
	}
