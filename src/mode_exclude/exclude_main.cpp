...
void exclude_main(std::vector < std::string > & argv) {
	...
	opt_parallel.add_options()
		("sample", boost::program_options::value< std::string >(), "sample to exclude")
        ("sample-filelist", boost::program_options::value< std::string >(), "file containing sample(s) to be excluded. One sample per line");
    ...
    if(P.options.count("s")){
        P.samplesToRemove.push_back(P.options["s"].as<std::string>());
