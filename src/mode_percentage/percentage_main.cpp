...
void percentage_main(std::vector < std::string > & argv)
{
	...
	opt_parallel.add_options()
        ...
        ("includePhen", boost::program_options::value< std::string>(), "Include phenotypes from bed file. Phenotypes ton include should be in bed format");
        ("excludePhen", boost::program_options::value< std::string>(), "Exclude phenotypes from bed file. Phenotypes to exclude should be in bed format");
    ...
    int nMode = P.options.count("missingness") + P.options.count("percentage") + P.options.count("includePhen") + P.options.count("excludePhen");
    if (nMode != 1) std::cout << "Please, specify only one of these options [--missingness, --percentage, --includePhen, --excludePhen]" << std::endl;
    ...
    if (P.options.count("includePhen")){
        ...
        P.readPhenotypesToInclude(P.options["includePhen"].as<std::string>());
        P.excludePhenotypes(P.options["bed"].as<std::string>(),outFile);
    }

    if (P.options.count("excludePhen")){
        P.mode = EXCLUDE_PHEN;
        std::cout << "  * Excluding phenotypes" << std::endl;
